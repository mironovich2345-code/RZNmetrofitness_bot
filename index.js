...
// ВЕРСИЯ 15.11.2025 — Supabase only (без db.json)
...
// Глобальный in-memory кэш (НЕ файл)
let db = {
  members: {},              // tg_id -> user
  referrals: [],            // события
  activity_submissions: [], // заявки
  news: [],
  schedules: {},
  promos: {},
  events: [],
  config: { next_submission_id: 1, last_reminders_run: 0 },
  assets: {},
  qr_cache: {},
  pending_purchase: {},
};

// Неблокирующие "обёртки" вместо файла
function loadDB(){ return db; }
function saveDB(){ /* no-op: всё уже в Supabase */ }

// Инициализация из Supabase
async function primeFromSupabase(){
  const { data: users } = await supabase.from('users').select('*');
  if (users) {
    for (const u of users) {
      db.members[Number(u.tg_id)] = {
        id: Number(u.tg_id),
        username: u.username,
        ...
        ui: u.ui || 'MAIN',
        ui_prev: u.ui_prev || 'MAIN',
        points: u.points || 0,
        tier: u.tier || 'Нет статуса',
        tmp_promo: null,
        bcast_draft: null,
        ref_last_shared_ts: 0,
        is_client: !!u.is_client,
        contract_no: null,
        contact_not: !!u.contact_not,
      };
    }
  }
  const { data: refs } = await supabase.from('referrals').select('*').order('created_at', { ascending:false });
  db.referrals = refs?.map(r => ({
    referrer_code: r.ref_code,
    invitee_id: Number(r.invitee_tg_id),
    stage: r.stage,
    phone: r.phone,
    ts: new Date(r.created_at).getTime()
  })) || [];

  const { data: acts } = await supabase.from('activity_submissions').select('*').order('ts', { ascending:false });
  db.activity_submissions = acts?.map(a => ({
    id: a.id,
    user_id: Number(a.user_id),
    type: a.type,
    evidence_type: a.evidence_type,
    evidence_value: a.evidence_value,
    status: a.status,
    approved_by: a.approved_by ? Number(a.approved_by) : null,
...
async function upsertUser(u){
  ...
  await supabase.from('users').upsert(row, { onConflict: 'tg_id' });
}

async function updatePhone(tgId, phone){
  await supabase.from('users').update({ phone, last_updated: new Date() }).eq('tg_id', String(tgId));
}

async function insertReferral(ref_code, invitee_tg_id, stage, phone=null){
  await supabase.from('referrals').insert({
    ref_code,
    invitee_tg_id: String(invitee_tg_id),
    stage,
    phone,
    created_at: new Date()
  });
...
async function insertActivity(a){
  await supabase.from('activity_submissions').insert(a);
}

async function updateActivity(id, patch){
  await supabase.from('activity_submissions').update(patch).eq('id', id);
}
...
async function notifyReferrerAboutAward(db, { referrer_id, refcode, invitee_id }, ctx){
  ...
  try { await tgSend.text(ref.id, text); return { ok:true }; }
  catch (e){ try { await tgSend.text(ctx.chat.id, `⚠️ Не удалось отправить уведомление пригласителю (${ref.id}): ${e.message||e}`); } catch(_){} return { ok:false, reason:'telegram_send_error' }; }
}
...
// DB helpers (под текущие вызовы в коде)
...
function ensureMember(db, from){
  if (!db.members[from.id]){
    ...
    upsertUser(db.members[from.id]); // fire-and-forget
  } else {
...
function setUI(db, uid, ui, opts={}){
  ...
  m.ui = ui;
  upsertUser(m); // persist
}
function goBack(db, uid){
  const m=ensureMember(db,{id:uid});
  const to=m.ui_prev||'MAIN';
  m.ui=to; m.ui_prev='MAIN'; upsertUser(m);
  return to;
}
...
const recalcUser=(db,u)=>{ u.points=calcPoints(db,u.id); u.tier=tierForPoints(u.points).name; upsertUser(u); };
const recalcAndSave=(db,u)=>{ recalcUser(db,u); };
...
bot.start(async (ctx)=>{
  ...
  if (payload){
    ...
    if(!existed){
      db.referrals.push({referrer_code:payload, invitee_id:ctx.from.id, stage:'started', phone:null, ts:Date.now()});
      saveDB(db);
      await insertReferral(payload, ctx.from.id, 'started', null);
    }
...
bot.hears('Приобрести абонемент', async (ctx)=>{
  ...
  await sendCard(ctx,'Оформление абонемента',['Пришлите одним сообщением:','ФИО; дата рождения (ДД.ММ.ГГГГ); номер телефона'], Markup.keyboard([['Назад']]).resize(),'banner_want');
});
...
async function sendReferralFast(ctx){
  ...
  me.ref_last_shared_ts=Date.now(); saveDB(db); upsertUser(me);
...
bot.hears(['Отзыв 2ГИС','Отзыв Яндекс','История ВК','Статус WA','История Telegram'], async (ctx)=>{
  ...
  const db=loadDB(); const m=ensureMember(db, ctx.from); m.ui_prev=m.ui; m.ui='SUBMIT_'+key; saveDB(db); upsertUser(m);
...
bot.on('text', async (ctx, next)=>{
  ...
  if (m.ui==='I_AM_CLIENT_CONTRACT'){
    m.contract_no=text; saveDB(db); upsertUser(m);
  ...
  if (m.ui && m.ui.startsWith('SUBMIT_')){
    ...
    const id=(db.config.next_submission_id = (db.config.next_submission_id||1)+1) - 1;
...
bot.action(/^approve_act_/i, async (ctx)=>{
  ...
  await updateActivity(id, { status:'approved', approved_by:String(ctx.from.id) });
  ...
  if (user){
    ...
    const after=tierForPoints(user.points); user.tier=after.name; saveDB(db); await upsertUser(user);
...
bot.action(/^reject_act_/i, async (ctx)=>{ try{ await ctx.answerCbQuery(); }catch(_){ }
  ...
  await updateActivity(id, { status:'rejected', approved_by:String(ctx.from.id) });
...
bot.action('pp_award', async (ctx)=>{
  ...
  if (inviteeId && refcode) {
    ...
    db.referrals.push({ referrer_code: refcode, invitee_id: inviteeId, stage: 'purchased', phone: st.phone, ts: Date.now() });
    saveDB(db);
    await insertReferral(refcode, inviteeId, 'purchased', st.phone || null);

    let ref = st.referrer_id ? db.members[st.referrer_id] : null;
    if (!ref) ref = Object.values(db.members).find(u => normCode(u.ref_code) === normCode(refcode)) || null;

    if (ref) {
      const before = ref.tier || 'Нет статуса';
      if (!had) { recalcAndSave(db, ref); await upsertUser(ref); }
      else { saveDB(db); await upsertUser(ref); }

      await notifyReferrerAboutAward(db, { referrer_id: ref.id, refcode, invitee_id: inviteeId }, ctx);
...
bot.hears(['Трейд-ин','День рождения','Студент'], async (ctx)=>{
  ...
  const key=promoKeyByTitle(ctx.message.text); if(!key) return; m.tmp_promo=key; saveDB(db); upsertUser(m);
...
bot.hears('Другой вопрос', async (ctx)=>{ const db=loadDB(); const m=ensureMember(db, ctx.from); m.ui_prev=m.ui; m.ui='ASK_FREE_TEXT'; saveDB(db); upsertUser(m); await sendCard(ctx,'Опишите вопрос','Одним сообщением — передадим менеджеру/тренеру.', Markup.keyboard([['Назад']]).resize(),'banner_support'); });
...
bot.on('contact', async (ctx)=>{
  ...
  await updatePhone(m.id, phone); await upsertUser(m);

  if (m.referred_by){
    const dupe=db.referrals.find(r=>r.referrer_code===m.referred_by && r.invitee_id===m.id && r.stage==='phone');
    if(!dupe){
      db.referrals.push({referrer_code:m.referred_by, invitee_id:m.id, stage:'phone', phone, ts:Date.now()}); saveDB(db);
      await insertReferral(m.referred_by, m.id, 'phone', phone);
    }
  }
...
bot.command('mark_phone', async (ctx)=>{
  ...
  await insertReferral(last.referrer_code, mem.id, stage, phone);
  if (stage==='purchased' && !hadPurchased){
    ...
    if (ref){
      const before = ref.tier || 'Нет статуса';
      recalcAndSave(db, ref); await upsertUser(ref);
...
bot.action(/^confirm_ref_/i, async (ctx)=>{
  ...
  saveDB(db);
  await insertReferral(refcode, inviteeId, 'purchased', null);

  const ref = Object.values(db.members).find(u => normCode(u.ref_code) === normCode(refcode)) || null;
  const before = ref ? (ref.tier || 'Нет статуса') : 'Нет статуса';

  if (!had && ref) { recalcAndSave(db, ref); await upsertUser(ref); }
  else { saveDB(db); if (ref) await upsertUser(ref); }

  await notifyReferrerAboutAward(db, { referrer_id: ref ? ref.id : null, refcode, invitee_id: inviteeId }, ctx);
...
bot.action(/^confirm_client_/i, async (ctx)=>{
  ...
  m.is_client = true; saveDB(db); await upsertUser(m);
...
bot.action(/^reject_client_/i, async (ctx)=>{
  ...
  m.is_client = false; saveDB(db); await upsertUser(m);
